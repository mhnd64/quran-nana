...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['import_quran'])) {
    ...
    if ($response === false) {
        $message = "Unable to connect to the external Quran API. Please check your internet connection or try again later.";
        $message_type = "error";
    } else {
        ...
        if ($data && $data['code'] === 200 && isset($data['data']['surahs'])) {
            ...
            $message = "Success! Loaded all $count verses directly into your local SQL database.";
            $message_type = "success";
        } else {
            $message = "Failed to parse data received from the external API.";
            $message_type = "error";
        }
    }
}
...
<head>
    ...
    <title>Holy Quran | Quran Analytics</title>
    ...
    <main class="layout">
        <section class="panel">
            <div class="section-heading">
                <div>
                    <h2>Browse & Search Holy Quran (SQL Database)</h2>
            ...
            <form method="GET" action="quran.php" class="controls-row">
                <div class="control-group">
                    ...
                    <select id="surah" name="surah" onchange="this.form.submit()">
                        <?php if (count($surahs_list) === 0): ?>
                            <option value="">No data (Please click the Download button above)</option>
                        <?php else: ?>
                            <?php foreach ($surahs_list as $s): ?>
                                <option value="<?php echo $s['surah_number']; ?>" <?php echo $selected_surah === (int)$s['surah_number'] && $search_query === '' ? 'selected' : ''; ?>>
                                    Surah <?php echo htmlspecialchars($s['surah_name']); ?> (<?php echo $s['surah_number']; ?>)
                                </option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                ...
                <div class="control-group">
                    <label for="search">Search Keywords (Uthmani Arabic or English Surah name)</label>
                    <input type="text" id="search" name="search" placeholder="Type search keyword... (e.g. الحديد, Al-Hadid)" value="<?php echo htmlspecialchars($search_query); ?>">
                </div>
            ...
            <div class="verses-container">
                    ...
                    <?php foreach ($verses as $v): ?>
                        <div class="verse-card">
                            <div class="verse-meta">
                                <span class="verse-badge">Surah <?php echo htmlspecialchars($v['surah_name']); ?></span>
                                <span>Surah: <?php echo $v['surah_number']; ?> | Verse: <?php echo $v['ayah_number']; ?></span>
                            </div>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Make sure you have typed a valid keyword or clicked on "Download Complete Quran" above to populate the database.</p>
                    </div>
                <?php endif; ?>
            </div>
        </section>
