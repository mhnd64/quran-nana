    ...
    if ($action === 'add') {
        ...
        if ($new_username !== '' && $new_password !== '') {
            ...
            if (mysqli_stmt_num_rows($check_stmt) > 0) {
                $message = __('username_taken_error');
                $message_type = "error";
            } else {
                ...
                if (mysqli_stmt_execute($insert_stmt)) {
                    $message = __('user_reg_success');
                    $message_type = "success";
                } else {
                    $message = __('user_reg_error');
                    $message_type = "error";
                }
...
if (isset($_GET['approve'])) {
    ...
    if ($approve_id > 0) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            $message = __('contributor_approved');
            $message_type = "success";
        } else {
            $message = __('contributor_approve_error');
            $message_type = "error";
        }
        mysqli_stmt_close($stmt);
    }
...
<head>
    ...
    <title><?php echo __('manage_users_title'); ?></title>
    ...
    <main class="layout">
        ...
        <section class="panel">
            <div class="section-heading">
                ...
                <p><?php echo __('register_user_privileges'); ?></p>
            </div>
            ...
            <form method="POST" action="manage_users.php">
                ...
                <div class="form-group">
                    <label for="role"><?php echo __('user_role_label'); ?></label>
                    <select id="role" name="role">
                        <option value="contributor"><?php echo __('role_contributor_option'); ?></option>
                        <option value="superadmin"><?php echo __('role_superadmin_option'); ?></option>
                    </select>
                </div>

                <button type="submit" class="btn-submit"><?php echo __('register_user_btn'); ?></button>
            </form>
        </section>
        ...
        <section class="panel">
            <div class="section-heading">
                <h2><?php echo __('system_user_directory'); ?></h2>
                <p><?php echo __('browse_verify_users'); ?></p>
            </div>
            ...
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th><?php echo __('username_th'); ?></th>
                            <th><?php echo __('access_level_th'); ?></th>
                            <th><?php echo __('status_th'); ?></th>
                            <th><?php echo __('reg_date_th'); ?></th>
                            <th><?php echo __('actions_th'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="role-tag role-<?php echo htmlspecialchars($u['role']); ?>">
                                            <?php echo htmlspecialchars($u['role'] === 'superadmin' ? __('role_superadmin') : __('role_contributor')); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="status-tag status-<?php echo htmlspecialchars($u['status']); ?>">
                                            <?php 
                                            if ($u['status'] === 'active') {
                                                echo htmlspecialchars(__('status_active'));
                                            } elseif ($u['status'] === 'suspended') {
                                                echo htmlspecialchars(__('status_suspended'));
                                            } else {
                                                echo htmlspecialchars(__('status_pending'));
                                            }
                                            ?>
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <div class="action-links">
                                            <?php if ($u['username'] === $_SESSION['username']): ?>
                                                <span style="color: var(--text-muted); font-size: 12px; font-style: italic;"><?php echo __('current_session'); ?></span>
                                            <?php else: ?>
                                                <?php if ($u['role'] === 'contributor'): ?>
                                                    <?php if ($u['status'] !== 'active'): ?>
                                                        <a href="manage_users.php?approve=<?php echo $u['id']; ?>" style="color: #10b981; font-weight: 700; text-decoration: none;"><?php echo __('approve_action'); ?></a>
                                                    <?php else: ?>
                                                        <a href="manage_users.php?suspend=<?php echo $u['id']; ?>" style="color: #f59e0b; font-weight: 700; text-decoration: none;"><?php echo __('suspend_action'); ?></a>
                                                    <?php endif; ?>
                                                <?php endif; ?>
                                                <a href="manage_users.php?delete=<?php echo $u['id']; ?>" class="action-link-delete" onclick="return confirm('<?php echo htmlspecialchars(__('confirm_delete_user'), ENT_QUOTES, 'UTF-8'); ?>')"><?php echo __('delete_action'); ?></a>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                </tr>
